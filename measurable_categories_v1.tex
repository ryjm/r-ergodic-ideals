...
\emph{\index{disjoint}disjoint} if $a \join b = 0$. For any set $W \in
...
\begin{defn}
  \label{def:16}
  A \emph{\index{projection band}projection band} is a band $B \subset
...
\begin{defn}[Band Projection]\label{def:11}
  A \emph{\index{band projection}band projection} onto $B$ is simply
...
\begin{thm}[Maximal Ergodic Theorem on Riesz Spaces]\label{thr:3}
  ...
  \emph{\index{nth maximal operator}nth maximal operator}, for $n \geq
  1$
...
\bibliography{measurable_categories_v1}\bibliographystyle{plain}
\printindex
\end{document}
