...
\begin{framed}
  ...
  \begin{defn}[order ideals and bands]\label{def:10}
    An \emph{\index{order ideal}order ideal} $I$ is a solid linear subspace of
    $V$. If every subset of this order ideal has its supremum (when it
    exists) in the order ideal itself, we call it a
    \emph{\index{band}band}. An order ideal generated by a non-empty subset
...
$\P$ on $(X, \X)$ we define the \emph{\index{ideal}ideal} to be the set
