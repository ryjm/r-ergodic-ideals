...
A \emph{\index{partially ordered set}partially ordered set}, or \emph{\index{poset}poset}, is a set $X$ equipped with the ordering $\leq$ that satisfies the following properties:
...
An \emph{\index{ordered vector space}ordered vector space} X is a vector space with the following properties:

\begin{deflist}{width-text}
\item X is partially ordered.
\item Vector space addition preserves order.
\item If $x \leq y$ then $\alpha x \leq \alpha y$. (\emph{\index{homogeneity}homogeneity})
\end{deflist}
\end{defn}

\begin{defn}\label{def:4}

A \emph{\index{lattice}lattice} is a poset $X$ where every two element subset has both a supremum and an infimum. For $x, y \in X$ denote $\sup(x,y)$ and $\inf(x,y)$ by $x \join y$ and $x \meet y$ respectively. 
\end{defn}
...
A \emph{\index{distributive lattice}distributive lattice} is a lattice where $x \meet (y \join w) = (x \meet y) \join (x \meet w)$. It can be shown that we can exchange $\meet$ and $\join$ as needed.
\end{defn}

\begin{defn}\label{def:7}
  A \emph{\index{Riesz space}Riesz space} is an ordered vector space that is also a lattice. In particular, this lattice is necessarily a distributive lattice.
...
A \emph{\index{Riesz subspace}Riesz subspace} $S$ of $E$ is a (in the sense of vector spaces) linear subspace of $E$ such that if $f,g \in S$ then $f \join g$ and $f \meet g$ are also in $S$.
\end{defn}

\begin{defn}\label{def:9}
 A \emph{\index{solid subset}solid subset} is a set $A \subset E$ such that if $f \in A$ and $|g| \leq |f|$, then $g \in A$.
\end{defn}

\begin{defn}\label{def:10}
An \emph{\index{ideal}ideal} is a solid linear subspace of $E$. An ideal \emph{generated} by $f\in E$ is the smallest ideal containing $f$, which is the set $I_{f} = \Set{g \in E: |g| \leq |\alpha f|}$, $\alpha \in \R$. 
\end{defn}

\begin{defn}\label{def:11}
A \emph{\index{band}band} is an ideal of $E$ such that every subset of the ideal has its supremum (when it exists in $E$) in the ideal. A band \emph{generated} by $f\in E$ is the smallest band containing $f$ (and thus contains $I_{\alpha}$ as well), and is called a \emph{\index{principal band}principal band}. An ideal generated by a non-empty subset $D$ of $E$ is the intersection of all ideals containing $D$ - similarly for a band.
\end{defn}

\begin{defn}\label{def:12}
An \emph{\index{order unit}order unit} is an element $f \in E$, $f > 0$ such that the ideal generated by $f$ is $E$. Similarly, $f$ is a \emph{\index{weak order unit}weak order unit} if the band generated by $f$ is $E$.
\end{defn}


\begin{defn}\label{def:13}
A set $H$ is called \emph{\index{upward directed}upward directed} if for any two elements $x,y \in H$ there exists a $z \in H$ such that $z \geq x \join y$. A set $H$ is called \emph{\index{downward directed}downward directed} if for any two elements $x,y \in H$ there exists a $z \in H$ such that $z \leq x \meet y$. A family of elements $f_{\alpha}$, $\alpha \in F$ where $F$ is some non-empty set, is also said to be \emph{upward directed} if for each $a,b \in F$ there exists $c \in F$ such that $f_c \geq f_a \join f_c$. If $f = \sup({f_\alpha})$ then we say that $f_{\alpha}$ is \emph{upward directed towards f} and we write $f_{\alpha}\uparrow f$. Downward directed is defined similarly. 
\end{defn}

\begin{defn}\label{def:14}
A linear operator $T : E \to D$ is said to be \emph{\index{order continuous}order continuous} if whenever $f_{\alpha} \downarrow 0$ we have $T(f_{\alpha}) \downarrow 0$.
\end{defn}
\begin{defn}\label{def:15}
A linear operator $T : E \to D$ is said to be \emph{\index{positive}positive} if whenever $f \geq 0$ we have $T(f) \geq 0$.
\end{defn}

\begin{defn}\label{def:16}
A \emph{\index{positive cone}positive cone} of $E$ is a subset $E^+ = \Set{f \in E : 0 \leq f}$.
\end{defn}


\begin{defn}[\index{Dedekind complete}Dedekind complete]\label{def:17}
...
Note that since $L^1(X,\X,\P)$ is complete, we must have that for any bounded set in $L^1(X,\X,\P)$ the supremum exists in $L^1(X,\X,\P)$. Thus $L^1(X,\X,\P)$ is a Dedekind complete Riesz space. This suggests that $\mathbf{E}$ should be a mapping on Dedekind complete Riesz spaces. Moreover, by (I), (III), and (IV), this mapping should be a \index{positive linear projection}positive linear projection on a Riesz subspace. (II) says that $\mathbf{E}$ should preserve weak order units, since the smallest ideal containing $\bar{1}$ in $L^1(X,\X,\P)$ is the entire space, and this ideal is a band since $L^1$ is Dedekind complete. Finally, (V) says that $\mathbf{E}$ should be order continuous. 

\begin{defn}[Conditional Expectation on Riesz Spaces]\label{def:18}
The \emph{\index{order conditional expectation}order conditional expectation} $\mathbf{E}: A \to B$, where $A$ is a Dedekind complete Riesz space with a weak order unit and $B \subset A$ is a Dedekind complete Riesz subspace, satisfies the following properties:
...
\begin{defn}[\index{band projection}Band Projection]\label{def:19}
...
Restricting $\mathbf{E}$ to a band projection onto the band generated by a \emph{\index{maximal operator}maximal operator} $M$ will give us the Maximal Ergodic Theorem.

\begin{thm}[Maximal Ergodic Theorem on Riesz Spaces]
\label{thr:1}
Let $E$ be a Riesz space with a weak order unit $e$. Let $\mathbf{E}$ be the conditional expectation operator on E such that $e = \mathbf{E}e$, and define the operator $T : E \to E$ to be positive with $\mathbf{E}|Tf| \leq \mathbf{E}|f|$, $f \in \mathbf{E}$. Define the \emph{\index{nth maximal operator}nth maximal operator}, for $n \geq 1$
\begin{equation}\label{eq:2}
M_n(f) = \sum\limits_{k=0}^{0} T^kf \join \sum\limits_{k=0}^{1} T^kf \join \cdots \join \sum\limits_{k=0}^{n-1} T^kf
\end{equation}

Now define the \emph{\index{maximal band}maximal band} $M(f)$ to be the band generated by $\Set{M_n(f)^+ | n \in \NN}$, and $J_{M(f)}$ to be the band projection onto the maximal band. Then we have
