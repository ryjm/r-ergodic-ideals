...
\begin{framed}
  ...
  (\textbf{Def:\ref{def:11}}), since the smallest ideal
  (\textbf{Def:\ref{def:10}}) containing $1$ in $L^1(V,\X,\P)$ is the
  entire space, and this ideal is a band (\textbf{Def:\ref{def:10}})
...
\emph{not} the case that either $a \leq b$ or $b \geq a$, $a,b \in
V$. We will not concern ourselves with other orderings for now.
...
into \emph{ideals} and \emph{bands}.
...
\begin{defn}[positive cone]\label{def:8}
  The \emph{\index{positive cone}positive cone} of $V$ is the subset $V^+
  = \Set{v \in V : 0 \leq v}$.
\end{defn}
...
\begin{framed}
  ...
  \begin{defn}[ideals and bands]\label{def:10}
    An \emph{\index{ideal}ideal} $I$ is a solid linear subspace of
    $V$. If every subset of this ideal has its supremum (when it
    exists) in the ideal itself, we call it a
    \emph{\index{band}band}. An ideal generated by a non-empty subset
    $U$ of $V$ is the intersection of all ideals containing $U$ -
    similarly for a band. An ideal \emph{generated} by $v \in V$ is
    the smallest ideal containing $v$, which is the set $I_{v} =
    \Set{w \in V: |w| \leq |\alpha v|}$, $\alpha \in \R$.
  \end{defn}

  \begin{rem}\label{rem:1}
    It turns out that an ideal is also a Riesz subspace. From
...
\end{framed}
So every band is an ideal, but not every ideal is a band. We can also
distinguish between ideals (bands) generated by single elements, or
\emph{principal ideals (bands)}. Furthermore, we can examine those
elements which can generate ideals (bands) equal to the space itself
...
\begin{framed}
  \begin{defn}[order unit]\label{def:11}
    ...
    > 0$ such that the ideal generated by $e$ is $V$. Similarly, $e$
    is a \emph{\index{weak order unit}weak order unit} if the band
    generated by $e$ is $V$.
  \end{defn}
